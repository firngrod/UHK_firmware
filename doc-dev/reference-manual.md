...
COMMAND = set secondaryRole.advanced.acceptTriggersFromSameHalf <false prevents secondary role keys from triggering as secondary from triggers from the same half of the keyboard (BOOL)>
...
- `setLedTxt <time> { STRING | VALUE }` will set led display to the supplemented text and block for the given time before updating display back to default value.
  ...
  - `notify STRING` displays a notification with the given text on the OLED display. The notification will automatically disappear after the default timeout (3 seconds). Only works on devices with OLED displays (UHK 80).
...
- `ifShortcut/ifNotShortcut/ifGesture/ifNotGesture [IFSHORTCUT_OPTIONS]* [KEYID]*` will wait for next keypresses until a sufficient number of keys have been pressed, either simultaneously (Shortcut) or in a sequence (Gesture). If the next keypresses correspond to the provided arguments (KEYIDs), the keypresses are consumed and the condition is executed. Both `if` and `ifNot` versions will consume the keypresses if the full list is matched; the `noConsume` option can be used to keep the remaining keys for further processing.
  - `Shortcut` requires simultaneous press of two or more keys (e.g., Ctrl+c), sometimes also called a *combo* or *chord*. When all the keys are pressed together, it executes the conditional command. It aborts when either the activation key is released, or a specific timeout is reached (`timeoutIn` or `cancelIn`). In the absence of timeouts, it can wait forever; there is no default timeout.
  - `Gesture` matches a tapped (non-simultaneous) sequence of keys (e.g., vim's gg). When all the keys are tapped in order, and within the timeout, it executes the conditional command. By default, the timeout is 1000 ms, and can be overridden with `timeoutIn` and `cancelIn`.
  - `IFSHORTCUT_OPTIONS`:
    - `noConsume` will prevent consumption of the additional keypresses. They will remain in the input queue and will be processed on their own. Useful if the next action(s) for those keys should still be taken.
    ...
    - `anyOrder` will check only presence of mentioned keyIds in postponer queue independent of their order.
